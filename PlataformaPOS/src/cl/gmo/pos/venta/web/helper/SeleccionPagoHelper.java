	...
	public SeleccionPagoForm cargaInicial(SeleccionPagoForm formulario, HttpSession session, String fecha_formulario)
	{
		...
		try {
			...
			if(formulario.getOrigen().equals(Constantes.STRING_PEDIDO))
			{
				...
				int total_prod = 0, cont_elib=0, importevg=0, porcentaje=0;
				
				formulario.setFpago_origen(session.getAttribute(Constantes.STRING_FORMA_PAGO_ORIGEN).toString());
				
				int por = NumberFormat.getNumberInstance().parse( session.getAttribute(Constantes.STRING_PORCENTAJE_ANTICIPO).toString()).intValue();
				...
				if((int)total_ventacr > 0 ) {
					porcentaje  = (int)((total_ventacr * 100) / totaln) + porcentaje;
				}else{
					porcentaje = por;
				}
